    ...
    def withdraw(amount, passwordinput):
        ...
        if len(wrongp_list) < 3:
            if passwordinput == password:
                ...
                wrongp_list.append(passwordinput)
                return 'Incorrect password'
        else:
            return "Your account is locked. Attempts: " + str(wrongp_list)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(error) == str:
        return error
    def joint(amount, passwordinput2):
        if passwordinput2 == new_password:
            return withdraw(amount, old_password)
        return withdraw(amount, passwordinput2)
    return joint
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) * upper_bound(y) > 0, 'y should not span zero'
...
def check_par():
    ...
    r2 = interval(2, 7) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    # f1 = a*lower_bound(x)*lower_bound(x) + b*lower_bound(x) + c
    # f2 = a*upper_bound(x)*upper_bound(x) + b*upper_bound(x) + c
    l, u, e = map(f, (lower_bound(x), upper_bound(x), extrem_point))
    if extrem_point < lower_bound(x) or extrem_point > upper_bound(x):
        return interval(min(l, u), max(l, u))
    else:
        if a < 0:
            return interval(min(l, u), e)
        else:
            return intreval(e, max(l, u))
