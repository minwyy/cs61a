...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        pre = scheme_eval(first, env)
        check_procedure(pre)
        if isinstance(pre, MacroProcedure):
            return scheme_eval(pre.apply_macro(rest, env), env)
        else:
            return scheme_apply(pre, rest.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 5
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('unfit arguments for the procedure')
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    formals, vals = Pair(bindings.first.first, nil), Pair(scheme_eval(bindings.first.second.first, env), nil)
    f, v = formals, vals
    while bindings.second is not nil:
        check_form(bindings.first, 2, 2)
        f.second, v.second = Pair(bindings.second.first.first, nil), Pair(scheme_eval(bindings.second.first.second.first, env), nil)
