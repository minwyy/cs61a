...
def find_centroid(cluster):
    ...
    return [mean(zip(*[restaurant_location(r) for r in cluster])[0]), mean(zip(*[restaurant_location(r) for r in cluster])[1])]
    # return [mean(zip(*(zip(*cluster)[1]))[0]), mean(zip((*zip(*cluster)[1]))[1])]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        centroids = [find_centroid(s) for s in group_by_centroid(restaurants, old_centroids)]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    pr = [find_predictor(user, reviewed, f) for f in feature_fns]
    # return pr[1][0]
    # return max(pr, key = lambda x: 3)[0]
    # print(max(pr, key = lambda x: x[1])[1])
    return max(pr, key = lambda x: x[1])[0]
    # END Question 8
